...
1. Fill your temperature dependant material properties in the `thermal.csv`, `mechanic.csv` & `plastic.csv` files.
  > ⚠️ Data must be ordered in ascending temperature.
  > 📌 Note : If you only want to run a thermal simulation, leave `mechanic.csv` and `plastic.csv` blank.
...
> ⚠️ Some changes — such as modifying the laser power — may cause compilation issues if the simulation increments are no longer suitable.  
In that case, consider adjusting the *Simulation* parameters in the Python file accordingly.
> ✏️ Custom : Feel free to change the MESH_SIZE in the *Simulation* parameters in the Python file if you want coarser or finer results, but keep in mind that a finer mesh will increase computation time.
...
5. Right-click the *Job* and select *Submit*.
  > 🔧 _Ignore all control panel prompts by clicking *Yes*._
  - It should be written "Submitted", then "Running".
  > 🔧 *Aborted* : _check `yourjobname.msg` or `yourjobname.dat` files to see from where the issues come_

6. Once "Completed", open your results in ABAQUS via: *Results > Output Databases > yourjobname.odb*

> 📌 Note : You can check the ongoing simulation before completion by opening the .odb file.
> 🔧 No worries If Abaqus closes while running, the job usually continues in the background (you can check if the .odb file size is increasing).
...
6. Open your terminal (e.g., Intel OneAPI 2024), and navigate to the folder containing your .inp file and .f90 subroutine: 
  ...
  > 📌 Note : If you don’t already have a Fortran compiler (supporting `.f90`), download one compatible version here:
...
7. Run the simulation on the terminal with: 
  ...
  > 🔧 *Abaqus/Analysis exited with errors* : _check `yourjobname.msg` or `yourjobname.dat` files to see from where the issues come_
...
> 🔧 The "laser penetration depth" parameter is not relevant here and can be left blank in the parameters panel.
...
> ⚠️ The first laser pass may appear not to melt the material completely. This is due to the part needing to adjust its body temperature.
...
> ✏️ Custom : In the `DFLUX` subroutine of the `.f90` file, you can: 
  - Enable heat loss due to evaporation (*Q_evap*) — effect is minor (~2 K lower).  
...
> ✏️ Custom : *H* corresponds to the laser penetration depth. You can adjust it to match the real melt pool depth. 
...
> ⚠️ Changing *H* affects both the shape and amplitude of the laser heat flux *Q_laser*, and therefore the overall temperature range.
...
This approach simulates the progressive deposition of molten metal using the *Birth–Death* method, commonly described in the literature.
...
>📌 Note: Each element’s size corresponds to the *h-spacing* (or wire diameter).
...
However:  
- In powder-bed fusion, it does not capture the powder → liquid → solid phase transformation or the interaction with surrounding powder.  
