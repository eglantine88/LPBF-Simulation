...
# Definir les differentes rayonnements du support & couches 
    ...
    for layer in range(NB_LAYERS):
        ...
        c1 = a.instances[f'Layer-{layer}'].cells[:]
        set += c1
        region = a.Set(cells=set, name=f'Alllayers-{layer}')

        # laser  top #
        for i in range(NB_LASER):   
            mdb.models['Model-1'].BodyHeatFlux(name=f'Laser-{layer}{i}', createStepName=f'Layer-{layer}Step-{i}', region=region, magnitude=1.0, distributionType=USER_DEFINED)
            if i == NB_LASER-1:
                if(layer < NB_LAYERS-1):
                    mdb.models['Model-1'].loads[f'Laser-{layer}{i}'].deactivate(f'Cooling-{layer}')
                else:
                    mdb.models['Model-1'].loads[f'Laser-{layer}{i}'].deactivate(f'Final-Cooling')
            else :
                mdb.models['Model-1'].loads[f'Laser-{layer}{i}'].deactivate(f'Cooling-{layer}Step-{i}')
        ...
        #mdb.models['Model-1'].SurfaceHeatFlux(name=f'Evaporation-{layer}', createStepName=f'Layer-{layer}Step-0', region=regiontop, magnitude=1.0, distributionType=USER_DEFINED)
        ...
        if layer != NB_LAYERS-1:
            ...
            #mdb.models['Model-1'].loads[f'Evaporation-{layer}'].deactivate(f'Layer-{layer+1}Step-0')
