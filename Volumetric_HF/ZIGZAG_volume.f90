! UMATHT code inspired by https://www.sciencedirect.com/science/article/pii/S2352492821002993
...
SUBROUTINE DFLUX(FLUX, SOL, KSTEP, KINC, TIME, NOEL, NPT, COORDS, JLTYP, TEMP, PRESS, SNAME)
  ...
  ! Variables
  DOUBLE PRECISION :: X0, Z0, X, Y, Z, T, TP, XC, YC, ZC, DIST2, R02, Q_laser, PSAT, H
  ...
  IF (JLTYP .EQ. 0) THEN ! Surface heat flux
  ! EVAPORATION
    TP = SOL(1)
    PSAT = 101325 * 10**(6.1127 - 18868 / TP) ! To change depending on the material
    FLUX(1) = - 0.82 * DHV * PSAT / SQRT((2.0 * M * PI * R * TP))
    
  ELSE ! Volumetric heat flux
  ! LASER

    H = -2*LASER_RADIUS + 300D-6
    ! Step info
    STEP = KSTEP-2
    ...
    IF (MOD(MODUL,2) .EQ. 0) THEN
      XC = X0 + LASER_SPEED * T
    ELSE
      XC = - X0 + LAYER_LENGTH - LASER_SPEED * T
    END IF
    ...
    IF (ABS(Y-YC) .LE. H) THEN
      DIST2 = (X - XC)**2 + (Z - ZC)**2
      R02 = LASER_RADIUS**2
      Q_laser = (LASER_POWER / (PI * R02 * H)) * EXP(- DIST2 / (1.5 * R02)) * (1-((Y-YC)/H)**2)
    ENDIF

    ! Other volumetric heat flux formulas
    ! Q_laser = (2 * 1.414 * LASER_POWER / ((PI**1.5) * R02 * H)) * EXP(-2 * DIST2 / R02) * EXP(- 2 * (Y-YC)**2 / H**2)
    ! Q_laser = (2 * LASER_POWER / ((PI**1.5) * R02 * H)) * EXP(-2 * DIST2 / R02) * EXP(- (Y-YC)**2 / H**2)
    ! Q_laser = (3 * LASER_POWER / (PI * R02 * H)) * EXP(-2 * DIST2 / R02) * (1 - (Y-YC)**2 / H**2)
    ! Q_laser = (LASER_POWER / (PI * R02 * H)) * EXP(-2 * DIST2 / R02) * EXP(- ABS(Y-YC) / H)
    ! Q_laser = 3*1.732*LASER_POWER/(R02*H*(PI**1.5)) * EXP(-3 * DIST2 / R02) * EXP(-3 * (Y-YC)**2 / H**2) 
    
    FLUX(1) = Q_laser*ABSORB
  ENDIF
  RETURN
